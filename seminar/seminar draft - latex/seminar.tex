...
\itemize{
    ...
    \item JSI will be an overhead when there is a lot of api's calls and heavy computation workloads needed to be done using api's.
    \item Since javascript is single threaded by design large computations can block the main thread.
    \item As the app size grows, the complexity of maintaining also increases exponentially.
}
...
\item \href{
    https://learn.microsoft.com/en-us/training/modules/build-mobile-and-desktop-apps/2-describe-maui-architecture?WT.mc_id=dotnet-35129-website&ns-enrollment-type=learningpath&ns-enrollment-id=learn.dotnet-maui.build-apps-with-dotnet-maui
}{
    MAUI [2]
}
% end of line 
